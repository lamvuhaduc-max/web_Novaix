...
export default function FeaturesSection({
  ...
  onChange: (newFeatures: FeaturesContent) => void;
}) {
  const [deleteItemIdx, setDeleteItemIdx] = useState<number | null>(null);

  const items = features.items || [];

  const updateField = <K extends keyof FeaturesContent>(key: K, val: FeaturesContent[K]) => {
    onChange({
      ...features,
      [key]: val,
    });
  ...
  const handleAddItem = () => {
    const next = [
      ...items,
      {
        n: items.length + 1,
        title: "Ưu điểm mới",
        desc: "Mô tả chi tiết về lợi thế cạnh tranh...",
      },
    ];
  ...
  const handleConfirmDeleteItem = () => {
    ...
    const next = items.filter((_, i) => i !== deleteItemIdx).map((it, idx) => ({ ...it, n: idx + 1 }));
    updateField("items", next);
  ...
  const handleMoveItem = (idx: number, dir: "up" | "down") => {
    ...
    const reindexed = next.map((it, i) => ({ ...it, n: i + 1 }));
    updateField("items", reindexed);
  };
  ...
  return (
    <Box>
      <FieldInput
        field={{ key: "kicker", path: "features.kicker", label: "Dòng nhãn", type: "text", max: 60, required: true }}
        value={features.kicker}
        onChange={(val) => updateField("kicker", val)}
      />

      <FieldInput
        field={{ key: "title", path: "features.title", label: "Tiêu đề", type: "text", max: 120, required: true }}
        value={features.title}
        onChange={(val) => updateField("title", val)}
      />

      <FieldInput
        field={{ key: "desc", path: "features.desc", label: "Mô tả", type: "textarea", max: 400 }}
        value={features.desc}
      ...
      {/* TÙY BIẾN MÀU SẮC RIÊNG CHO KHỐI VÌ SAO CHỌN */}
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
        <Box>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "13.5px" }}>
            Tùy chỉnh màu sắc riêng cho Vì sao chọn OAlpha
          </Typography>
          <Typography variant="caption" sx={{ color: "text.secondary", display: "block" }}>
            {features.customColors ? "Đang áp dụng bộ màu riêng cho khối này" : "Đang kế thừa tự động theo Màu Toàn Trang"}
          </Typography>
        </Box>
        <FormControlLabel
          control={
            <Switch
              checked={Boolean(features.customColors)}
        ...
        <Box sx={{ bgcolor: "background.default", p: 1.5, borderRadius: "10px", border: "1px solid", borderColor: "divider", mb: 2.5 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography variant="caption" sx={{ fontWeight: 700, color: "primary.main", letterSpacing: "0.05em" }}>
              BẢNG MÀU ĐỘC BẢN VÌ SAO CHỌN OALPHA
            </Typography>
            <Button
              size="small"
          ...
          <ColorInput
            ...
            value={features.kickerColor || "#2dd4bf"}
            onChange={(val) => updateField("kickerColor", val)}
          />
          <ColorInput
            label="Màu chữ tiêu đề & Ưu điểm"
            value={features.titleColor || "#eef2fb"}
            onChange={(val) => updateField("titleColor", val)}
          />
          <ColorInput
            label="Màu chữ mô tả"
            value={features.descColor || "#9aa6c4"}
            onChange={(val) => updateField("descColor", val)}
          />
          <ColorInput
            label="Màu nền khối"
            value={features.bgColor || "#0b1120"}
            onChange={(val) => updateField("bgColor", val)}
          />
          <ColorInput
            label="Màu nền thẻ ưu điểm"
            value={features.cardBgColor || "#131c31"}
            onChange={(val) => updateField("cardBgColor", val)}
          />
          <ColorInput
            label="Màu số thứ tự & Huy hiệu trung tâm"
            value={features.badgeColor || "#2dd4bf"}
            onChange={(val) => updateField("badgeColor", val)}
          />
        </Box>
      ) : (
      ...
      <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "14px", mb: 1.5, color: "primary.main" }}>
        Danh sách ưu điểm ({items.length}/8)
      </Typography>
            ...
            <AccordionDetails sx={{ p: 2, pt: 1.5, borderTop: "1px solid", borderColor: "divider", bgcolor: "background.default" }}>
              <Box sx={{ mb: 1.5 }}>
                ...
                <TextField
                  ...
                  fullWidth
                  value={it.title || ""}
                  placeholder="Ví dụ: Một nguồn dữ liệu duy nhất..."
      ...
      <Dialog open={deleteItemIdx !== null} onClose={() => setDeleteItemIdx(null)}>
        <DialogTitle sx={{ fontWeight: 600, fontSize: "16px" }}>Xác nhận xóa ưu điểm</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ fontSize: "14px" }}>
            Bạn có chắc muốn xóa ưu điểm <b>&ldquo;{deleteItemIdx !== null ? items[deleteItemIdx]?.title : ""}&rdquo;</b> không?
          </DialogContentText>
        </DialogContent>
