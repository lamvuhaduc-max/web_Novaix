...
export default function ModulesSection({
  ...
  onChange: (newModules: ModulesContent) => void;
}) {
  const [deleteItemIdx, setDeleteItemIdx] = useState<number | null>(null);

  const items = modules.items || [];

  const updateField = <K extends keyof ModulesContent>(key: K, val: ModulesContent[K]) => {
    onChange({
      ...modules,
      [key]: val,
    });
  ...
  const handleAddItem = () => {
    const next = [
      ...items,
      {
        icon: "⚡",
        title: "Module mới",
        desc: "Mô tả tính năng và giá trị nghiệp vụ của module này mang lại cho doanh nghiệp...",
        tag: "Tiện ích mở rộng",
      },
    ];
  ...
  return (
    <Box>
      {/* Thông tin đầu khối Modules */}
      <FieldInput
        field={{ key: "kicker", path: "modules.kicker", label: "Dòng nhãn", type: "text", max: 60, required: true }}
        value={modules.kicker}
        onChange={(val) => updateField("kicker", val)}
      />

      <FieldInput
        field={{ key: "title", path: "modules.title", label: "Tiêu đề", type: "text", max: 120, required: true }}
        value={modules.title}
        onChange={(val) => updateField("title", val)}
      />

      <FieldInput
        field={{ key: "desc", path: "modules.desc", label: "Mô tả", type: "textarea", max: 400 }}
        value={modules.desc}
      ...
      {/* TÙY BIẾN MÀU SẮC RIÊNG CHO KHỐI MODULES */}
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
        <Box>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "13.5px" }}>
            Tùy chỉnh màu sắc riêng cho Module
          </Typography>
          <Typography variant="caption" sx={{ color: "text.secondary", display: "block" }}>
            {modules.customColors ? "Đang áp dụng bộ màu riêng cho khối Module" : "Đang kế thừa tự động theo Màu Toàn Trang"}
          </Typography>
        </Box>
        <FormControlLabel
          control={
            <Switch
              checked={Boolean(modules.customColors)}
        ...
        <Box sx={{ bgcolor: "background.default", p: 1.5, borderRadius: "10px", border: "1px solid", borderColor: "divider", mb: 2.5 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography variant="caption" sx={{ fontWeight: 700, color: "primary.main", letterSpacing: "0.05em" }}>
              BẢNG MÀU ĐỘC BẢN HỆ SINH THÁI MODULE
            </Typography>
            <Button
              size="small"
          ...
          <Typography variant="caption" sx={{ fontWeight: 600, color: "text.secondary", mb: 1, display: "block" }}>
            Tiêu đề & Nhãn khối Module
          </Typography>
          <ColorInput
            label="Màu nhãn trên (Kicker)"
            value={modules.kickerColor || "#2dd4bf"}
            onChange={(val) => updateField("kickerColor", val)}
          />
          <ColorInput
            label="Màu chữ tiêu đề chính"
            value={modules.titleColor || "#eef2fb"}
            onChange={(val) => updateField("titleColor", val)}
          />
          <ColorInput
            label="Màu chữ mô tả chính"
            value={modules.descColor || "#9aa6c4"}
            onChange={(val) => updateField("descColor", val)}
          />
          <ColorInput
            label="Màu nền khối Module"
            value={modules.bgColor || "#070b16"}
          ...
          <ColorInput
            label="Màu nền thẻ module"
            value={modules.cardBgColor || "#0d1424"}
            onChange={(val) => updateField("cardBgColor", val)}
          />
          <ColorInput
            label="Màu tên module (Card Title)"
            value={modules.cardTitleColor || "#eef2fb"}
            onChange={(val) => updateField("cardTitleColor", val)}
          />
          <ColorInput
            label="Màu mô tả module (Card Desc)"
            value={modules.cardDescColor || "#9aa6c4"}
            onChange={(val) => updateField("cardDescColor", val)}
          />
          <ColorInput
            label="Màu nhãn phân loại (Tag)"
            value={modules.tagColor || "#38bdf8"}
            onChange={(val) => updateField("tagColor", val)}
          />
        </Box>
      ) : (
        <Alert severity="info" sx={{ mb: 2.5, py: 0.5, fontSize: "12px", borderRadius: "8px" }}>
          Khối Module đang tự động kế thừa bảng màu chung. Khi bạn đổi Bảng màu toàn trang, khối này sẽ tự động đổi màu đồng bộ.
        </Alert>
      )}

      <Divider sx={{ my: 2.5 }} />

      {/* DANH SÁCH CÁC MODULE NGHIỆP VỤ */}
      <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "14px", mb: 1.5, color: "primary.main" }}>
        Danh sách Module nghiệp vụ ({items.length}/16)
      </Typography>
          ...
          >
            ...
            >
              <Typography variant="subtitle2" sx={{ fontWeight: 600, fontSize: "13px" }}>
                {m.icon || "📦"} {m.title || "Module"} ({m.tag || "Tag"})
              </Typography>
            ...
            <AccordionDetails sx={{ p: 2, pt: 1.5, borderTop: "1px solid", borderColor: "divider", bgcolor: "background.default" }}>
              <Box sx={{ display: "flex", gap: 1, mb: 1.5 }}>
                <Box>
                  ...
                  <TextField
                    ...
                    sx={{ width: 75, "& .MuiOutlinedInput-root": { borderRadius: "6px", bgcolor: "background.paper" } }}
                  />
                </Box>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <TextField
                    ...
                    fullWidth
                    value={m.title || ""}
                    placeholder="Ví dụ: CRM — Quan hệ khách hàng..."
              ...
              <Box sx={{ mb: 1.5 }}>
                ...
                <TextField
                  ...
                  fullWidth
                  value={m.tag || ""}
                  placeholder="Ví dụ: Sales · Marketing · CSKH..."
              ...
              <Box>
                ...
                <TextField
                  ...
                  rows={2}
                  value={m.desc || ""}
                  placeholder="Mô tả chức năng chính của module..."
      ...
      <Dialog open={deleteItemIdx !== null} onClose={() => setDeleteItemIdx(null)}>
        <DialogTitle sx={{ fontWeight: 600, fontSize: "16px" }}>Xác nhận xóa module</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ fontSize: "14px" }}>
            Bạn có chắc muốn xóa module <b>&ldquo;{deleteItemIdx !== null ? items[deleteItemIdx]?.title : ""}&rdquo;</b> không?
          </DialogContentText>
        </DialogContent>
