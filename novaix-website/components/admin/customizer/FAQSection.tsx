...
export default function FAQSection({
  ...
  faq: FAQContent;
  onChange: (newFAQ: FAQContent) => void;
}) {
  const [deleteItemIdx, setDeleteItemIdx] = useState<number | null>(null);

  const items = faq.items || [];

  const updateField = <K extends keyof FAQContent>(key: K, val: FAQContent[K]) => {
    onChange({
      ...faq,
      [key]: val,
    });
  ...
  const handleAddItem = () => {
    const next = [
      ...
      {
        q: "Câu hỏi mới?",
        a: "Câu trả lời chi tiết và rõ ràng cho câu hỏi này...",
      },
    ];
  ...
  return (
    <Box>
      <FieldInput
        field={{ key: "kicker", path: "faq.kicker", label: "Dòng nhãn", type: "text", max: 60, required: true }}
        value={faq.kicker}
        onChange={(val) => updateField("kicker", val)}
      />

      <FieldInput
        field={{ key: "title", path: "faq.title", label: "Tiêu đề", type: "text", max: 120, required: true }}
        value={faq.title}
        onChange={(val) => updateField("title", val)}
      />

      <FieldInput
        field={{ key: "desc", path: "faq.desc", label: "Mô tả", type: "textarea", max: 400 }}
        value={faq.desc}
      ...
      {/* TÙY BIẾN MÀU SẮC RIÊNG CHO KHỐI FAQ */}
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
        <Box>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "13.5px" }}>
            Tùy chỉnh màu sắc riêng cho Câu hỏi thường gặp
          </Typography>
          <Typography variant="caption" sx={{ color: "text.secondary", display: "block" }}>
            {faq.customColors ? "Đang áp dụng bộ màu riêng cho khối này" : "Đang kế thừa tự động theo Màu Toàn Trang"}
          </Typography>
        </Box>
        <FormControlLabel
          control={
            <Switch
              checked={Boolean(faq.customColors)}
        ...
        <Box sx={{ bgcolor: "background.default", p: 1.5, borderRadius: "10px", border: "1px solid", borderColor: "divider", mb: 2.5 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography variant="caption" sx={{ fontWeight: 700, color: "primary.main", letterSpacing: "0.05em" }}>
              BẢNG MÀU ĐỘC BẢN CÂU HỎI THƯỜNG GẶP (FAQ)
            </Typography>
            <Button
              size="small"
          ...
          <ColorInput
            ...
            value={faq.kickerColor || "#2dd4bf"}
            onChange={(val) => updateField("kickerColor", val)}
          />
          <ColorInput
            label="Màu chữ tiêu đề & Câu hỏi thường"
            value={faq.titleColor || "#eef2fb"}
            onChange={(val) => updateField("titleColor", val)}
          />
          <ColorInput
            label="Màu chữ mô tả & Câu trả lời"
            value={faq.descColor || "#9aa6c4"}
            onChange={(val) => updateField("descColor", val)}
          />
          <ColorInput
            label="Màu nền khối FAQ"
            value={faq.bgColor || "#0b1120"}
            onChange={(val) => updateField("bgColor", val)}
          />
          <ColorInput
            label="Màu nền thẻ câu hỏi"
            value={faq.cardBgColor || "#131c31"}
            onChange={(val) => updateField("cardBgColor", val)}
          />
          <ColorInput
            label="Màu câu hỏi khi đang mở (Active)"
            value={faq.activeQuestionColor || "#2dd4bf"}
            onChange={(val) => updateField("activeQuestionColor", val)}
          />
        </Box>
      ) : (
      ...
      <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "14px", mb: 1.5, color: "primary.main" }}>
        Danh sách câu hỏi ({items.length}/20)
      </Typography>
            ...
            <AccordionDetails sx={{ p: 2, pt: 1.5, borderTop: "1px solid", borderColor: "divider", bgcolor: "background.default" }}>
              <Box sx={{ mb: 1.5 }}>
                ...
                <TextField
                  ...
                  fullWidth
                  value={it.q || ""}
                  placeholder="Ví dụ: Thời gian triển khai mất bao lâu?..."
      ...
      <Dialog open={deleteItemIdx !== null} onClose={() => setDeleteItemIdx(null)}>
        <DialogTitle sx={{ fontWeight: 600, fontSize: "16px" }}>Xác nhận xóa câu hỏi</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ fontSize: "14px" }}>
            Bạn có chắc muốn xóa câu hỏi <b>&ldquo;{deleteItemIdx !== null ? items[deleteItemIdx]?.q : ""}&rdquo;</b> không?
          </DialogContentText>
        </DialogContent>
