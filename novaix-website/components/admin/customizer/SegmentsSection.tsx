...
export default function SegmentsSection({
  ...
  onChange: (newSegments: SegmentsContent) => void;
}) {
  const [deleteItemIdx, setDeleteItemIdx] = useState<number | null>(null);

  const items = segments.items || [];

  const updateField = <K extends keyof SegmentsContent>(key: K, val: SegmentsContent[K]) => {
    onChange({
      ...segments,
      [key]: val,
    });
  ...
  const handleAddItem = () => {
    const next = [
      ...items,
      {
        icon: "💡",
        title: "Đối tượng mới",
        desc: "Mô tả giải pháp cho nhóm khách hàng này...",
        items: ["Đặc điểm 1", "Đặc điểm 2", "Đặc điểm 3"],
      },
    ];
  ...
  return (
    <Box>
      <FieldInput
        field={{ key: "kicker", path: "segments.kicker", label: "Dòng nhãn", type: "text", max: 60, required: true }}
        value={segments.kicker}
        onChange={(val) => updateField("kicker", val)}
      />

      <FieldInput
        field={{ key: "title", path: "segments.title", label: "Tiêu đề", type: "text", max: 120, required: true }}
        value={segments.title}
        onChange={(val) => updateField("title", val)}
      />

      <FieldInput
        field={{ key: "desc", path: "segments.desc", label: "Mô tả", type: "textarea", max: 400 }}
        value={segments.desc}
      ...
      {/* TÙY BIẾN MÀU SẮC RIÊNG CHO KHỐI ĐỐI TƯỢNG */}
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
        <Box>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "13.5px" }}>
            Tùy chỉnh màu sắc riêng cho Đối tượng
          </Typography>
          <Typography variant="caption" sx={{ color: "text.secondary", display: "block" }}>
            {segments.customColors ? "Đang áp dụng bộ màu riêng cho khối này" : "Đang kế thừa tự động theo Màu Toàn Trang"}
          </Typography>
        </Box>
        <FormControlLabel
          control={
            <Switch
              checked={Boolean(segments.customColors)}
        ...
        <Box sx={{ bgcolor: "background.default", p: 1.5, borderRadius: "10px", border: "1px solid", borderColor: "divider", mb: 2.5 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography variant="caption" sx={{ fontWeight: 700, color: "primary.main", letterSpacing: "0.05em" }}>
              BẢNG MÀU ĐỘC BẢN GIẢI PHÁP THEO ĐỐI TƯỢNG
            </Typography>
            <Button
              size="small"
          ...
          <ColorInput
            ...
            value={segments.kickerColor || "#2dd4bf"}
            onChange={(val) => updateField("kickerColor", val)}
          />
          <ColorInput
            label="Màu chữ tiêu đề & Tên nhóm"
            value={segments.titleColor || "#eef2fb"}
            onChange={(val) => updateField("titleColor", val)}
          />
          <ColorInput
            label="Màu chữ mô tả"
            value={segments.descColor || "#9aa6c4"}
            onChange={(val) => updateField("descColor", val)}
          />
          <ColorInput
            label="Màu nền khối Đối tượng"
            value={segments.bgColor || "#0b1120"}
            onChange={(val) => updateField("bgColor", val)}
          />
          <ColorInput
            label="Màu nền thẻ đối tượng"
            value={segments.cardBgColor || "#131c31"}
            onChange={(val) => updateField("cardBgColor", val)}
          />
          <ColorInput
            label="Màu mũi tên điểm nhấn (›)"
            value={segments.accentColor || "#38bdf8"}
        ...
        {items.map((s, idx) => (
          ...
          >
            ...
            <AccordionDetails sx={{ p: 2, pt: 1.5, borderTop: "1px solid", borderColor: "divider", bgcolor: "background.default" }}>
              <Box sx={{ display: "flex", gap: 1, mb: 1.5 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <TextField
                    ...
                    fullWidth
                    value={s.title || ""}
                    placeholder="Ví dụ: SME đang tăng trưởng..."
              ...
              <Box>
                <Typography variant="caption" sx={{ fontWeight: 600, mb: 0.5, display: "block" }}>
                  Các đặc điểm / lợi ích nổi bật (mỗi dòng 1 ý) *
                </Typography>
                <TextField
                  size="small"
                  fullWidth
                  ...
                  rows={3}
                  value={(s.items || []).join("\n")}
                  placeholder="Nhập mỗi đặc điểm trên 1 dòng..."
                  onChange={(e) => {
                    const lines = e.target.value.split("\n").filter((l) => l.trim().length > 0);
                    handleItemChange(idx, "items", lines.length > 0 ? lines : ["Đặc điểm mới"]);
                  }}
                  sx={{ "& .MuiOutlinedInput-root": { borderRadius: "6px", bgcolor: "background.paper", fontSize: "12.5px" } }}
                />
              </Box>
      ...
      <Dialog open={deleteItemIdx !== null} onClose={() => setDeleteItemIdx(null)}>
        <DialogTitle sx={{ fontWeight: 600, fontSize: "16px" }}>Xác nhận xóa đối tượng</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ fontSize: "14px" }}>
            Bạn có chắc muốn xóa nhóm đối tượng <b>&ldquo;{deleteItemIdx !== null ? items[deleteItemIdx]?.title : ""}&rdquo;</b> không?
          </DialogContentText>
        </DialogContent>
